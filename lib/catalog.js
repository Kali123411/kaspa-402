// lib/catalog.js — shared marketplace metadata + helpers, used by both the UI and the API routes
// (/llms.txt, /llms.json, /skill/<cap>) so they never drift.
export const GATEWAY = 'https://x402-compute.68cxgfyr0.workers.dev';
export const MCP_URL = GATEWAY + '/mcp';
export const REGISTRY_POST = GATEWAY + '/registry/list';
export const SITE = 'https://kaspa-402.org';

export const fmt = (n) => (n >= 100 ? n.toFixed(0) : n >= 10 ? n.toFixed(1) : n.toFixed(2));
...
// how an agent pays — reused as plain lines (llms.txt) and structured (llms.json)
export const PAY = {
  mcp: `claude mcp add --transport http k402 ${MCP_URL}`,
  session: `POST ${GATEWAY}/onboard/request for a Kaspa deposit address; fund it once with KAS, then call any endpoint with header X-Session: <session> (metered per token).`,
  channel: `pip install k402; open a covenant payment channel to a provider and pay per call with off-chain vouchers (k402.ChannelPayer). No custodian.`,
};
export const DISCOVERY = {
  services: `${GATEWAY}/registry/search`,
  filter: `${GATEWAY}/registry/search?capability=<cap>&max_price_usd=<n>&min_reputation_kas=<n>`,
  provider: `${GATEWAY}/registry/provider/<payee_pubkey>`,
};
...
// Endpoints the cached /api/health sweep confirmed dead. Fail-open: any error (or unchecked
// endpoint) yields an empty set, so we never hide a live listing on a health hiccup.
export async function deadEndpoints(baseUrl) {
  try {
    const h = await (await fetch(`${baseUrl}/api/health`)).json();
    const dead = new Set();
    for (const [ep, v] of Object.entries(h.endpoints || {})) if (v && v.alive === false) dead.add(ep);
    return dead;
  } catch (e) {
    return new Set();
  }
