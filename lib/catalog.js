...
export const CAP_INFO = {
  ...
  'kaspa-finality-proof': { what: 'Succinct proof that a Kaspa transaction is FINAL and DAG-accepted — a sparse-anchor light-client proof (k=49 sampled PoW anchors over an ~86,000-block burial window) wrapped as a 356-byte SP1 Groth16 proof that Ethereum verifies on-chain with SP1\'s stock verifier. Asynchronous: the paid call returns a ticket, the proof follows. The transaction must already be buried (~16h) before a proof can exist.', send: '{ "burnTxid": "…", "recip": "0x…", "amount": "…", "nonce": "…", "proverAddr": "0x… (optional)" }', get: '{ "ticket": "…", "poll": "https://…/proof/<ticket>" } then { "proof": "0x… (356B)", "publicValues": "0x… (430B)" }' },
...
export const CATEGORIES = [
  ...
  ['Zero-knowledge', (c) => c === 'zk-prove' || c === 'attest' || c === 'kaspa-finality-proof'],
