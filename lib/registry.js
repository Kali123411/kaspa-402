...
import { SEED, BASE_SEED } from './listings.js';
import { validateListing, meetsPriceFloor } from './x402catalog.js';
...
export async function getListings() {
  ...
  // The 0.5 KAS price floor applies to every listing — curated seed and open submissions alike —
  // so the directory never shows a dust/spam-prone (sub-floor) service. `kind` defaults to 'kaspa'
  // (Kaspa-native); BASE_SEED entries carry kind:'base' (settled on Base via the router).
  return [...SEED, ...BASE_SEED, ...subs]
    .map((l) => ({ kind: 'kaspa', ...l }))
    .filter((l) => meetsPriceFloor(l) && validateListing(l).ok);
}

export async function addSubmission(listing) {
  const v = validateListing(listing);
  if (!v.ok) return { ok: false, error: v.error };
  const entry = { ...listing, id: listing.id || `sub_${Date.now().toString(36)}`, source: 'submission', verified: listing.verified === true, submittedAt: Date.now() };
  const r = await redis();
