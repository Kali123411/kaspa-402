...
// Send one unpaid GET (x402 resources are GET) and return the raw shape. Throws on SSRF/connection/timeout.
export async function probe(rawUrl, { timeout = 9000 } = {}) {
  ...
  try {
    ...
    const headers = {};
    for (const [k, v] of r.headers) headers[k.toLowerCase()] = v;   // x402 v2 offer rides in PAYMENT-REQUIRED
    const text = await r.text().catch(() => '');
    let json = null;
    try { json = JSON.parse(text); } catch { /* not json */ }
    return { href: url.href, status: r.status, latency, headers, json, textLen: text.length };
  } finally {
    clearTimeout(t);
  }
}
