...
const chatModel = (alias, name, capability, note) => ({
  ...
  description: `${note} Append \`?prompt=…\` to the URL; each call settles 0.5 KAS via x402 \`exact\` (standard-native) on mainnet and returns a reply.`,
  capability,
  scheme: 'exact',
...
export const SEED = [
  ...
  chatModel('uncensored', 'uncensored', 'llm:chat', 'Unfiltered chat on Qwen3.6-35B (p920rig).'),
  {
    ...
    description: 'Text embeddings on nomic-embed-text (p920rig). Append `?input=…`; each call settles 0.5 KAS via x402 `exact` on mainnet and returns a 768-dim vector.',
    capability: 'embed',
    scheme: 'exact',
    network: 'kaspa:mainnet',
    amountSompi: 50000000,
