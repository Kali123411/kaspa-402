...
// One mainnet chat model, selected via ?model= on the shared chat gateway.
const chatModel = (alias, name, capability, note) => ({
  id: `kaspa-402-chat-${alias}`,
  resource: `${CHAT}?model=${alias}`,
  serviceName: `Kaspa·402 — ${name}`,
  description: `${note} Append \`?prompt=…\` to the URL; each call settles $0.05 (paid in KAS at the current rate, min 0.5 KAS) via x402 \`exact\` (standard-native) on mainnet and returns a reply.`,
  capability,
  scheme: 'exact',
...
export const SEED = [
  ...
  chatModel('chat', 'chat', 'llm:chat', 'General chat on llama3.1:8b (murrbsmt2).'),
  chatModel('fast', 'fast chat', 'llm:chat', 'Low-latency chat on qwen2.5:3b (p920rig).'),
  chatModel('chat-mini', 'chat-mini', 'llm:chat', 'Balanced chat on qwen2.5:7b (p920rig).'),
  chatModel('reason', 'reason', 'llm:reason', 'Deeper reasoning on qwen3.5:35b (murrbsmt2).'),
  chatModel('think', 'think', 'llm:reason', 'Reasoning-tuned qwen-think (murrbsmt2).'),
  chatModel('uncensored', 'uncensored', 'llm:chat', 'Unfiltered chat on Qwen3.6-35B (p920rig).'),
  {
    id: 'kaspa-402-embed',
    resource: 'https://kaspa-402-embed.kaspadev.workers.dev/exact',
    serviceName: 'Kaspa·402 — embed',
    description: 'Text embeddings on nomic-embed-text (p920rig). Append `?input=…`; each call settles $0.05 (paid in KAS at the current rate, min 0.5 KAS) via x402 `exact` on mainnet and returns a 768-dim vector.',
    capability: 'embed',
    scheme: 'exact',
    network: 'kaspa:mainnet',
    priceUsd: 0.05,
