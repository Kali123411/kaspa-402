...
export const SEED = [
  ...
  chatModel('chat', 'chat', 'llm:chat', 'General chat on llama3.1:8b (murrbsmt2).', 0.03),
  chatModel('fast', 'fast chat', 'llm:chat', 'Low-latency chat on qwen2.5:3b (p920rig).', 0.01),
  chatModel('chat-mini', 'chat-mini', 'llm:chat', 'Balanced chat on qwen2.5:7b (p920rig).', 0.01),
  chatModel('reason', 'reason', 'llm:reason', 'Deeper reasoning on qwen3.5:35b (murrbsmt2).', 0.08),
  chatModel('think', 'think', 'llm:reason', 'Reasoning-tuned qwen-think (murrbsmt2).', 0.10),
  chatModel('uncensored', 'uncensored', 'llm:chat', 'Unfiltered chat on Qwen3.6-35B (p920rig).', 0.08),
  {
    id: 'kaspa-402-embed',
    resource: 'https://kaspa-402-embed.kaspadev.workers.dev/exact',
    serviceName: 'Kaspa·402 — embed',
    description: 'Text embeddings on nomic-embed-text (p920rig). Append `?input=…`; each call settles $0.01 (paid in KAS at the current rate, min 0.5 KAS) via x402 `exact` on mainnet and returns a 768-dim vector.',
    capability: 'embed',
    scheme: 'exact',
    network: 'kaspa:mainnet',
    priceUsd: 0.01,
